...
config_args['LAYERS'] = 12
config_args['N_HEAD'] = 12
config_args['EMBED'] = 768
...
for k in config_.keys():
    print("Forcing change in ", k, config2_dict[k], '->', config_[k])
...
#    model_args['VOCAB'] = meta_vocab_size if meta_vocab_size is not None else 50304
    #gptconf = GPTConfig(**model_args)
    #model = GPT2Model(gptconf)

    print("The final config before instantiating the model is:")
   ...
   while True:  
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5: # let the training loop settle a bit
            mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt,config)
