...
#        print("local rank",ddp_local_rank,"wt",model.mlp.experts.c_fc.weight[0][0])
    ...
    if ddp_local_rank ==  0 : 
        print("output shape,mag ",output.shape,' ',torch.norm(output),'target ',torch.norm(output1),"loss",loss.item(),'rank',ddp_local_rank)
...
#    raw_model = model.module if ddp else model # unwrap DDP container if needed
    ...
    while True:
        ...
        with ctx:
            ...
            if ddp_local_rank ==  0 : 
                print("output shape,mag ",logits.shape,' ',torch.norm(logits),'target ',torch.norm(Y),"loss",loss.item(),'rank',ddp_local_rank)
            ## for 10 random embeds in X abve, check if Y is correct
            if iter_num % max_iters_check_part == 0:
                for i in range(10):
                    sample = random.randint(0, config_.BLOCK_SIZE-1)
                    x  = X[0][sample]
                    y = Y[0][sample]
                    err = torch.norm(logits[0][sample] - y)
                    print(f"Training-Rank {ddp_local_rank}, y is {torch.norm(y)}, logits is {torch.norm(logits[0][sample])}, err is {err}, % error is {err/torch.norm(y)*100}")
